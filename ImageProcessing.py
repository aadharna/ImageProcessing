...
class ImageProcessor:
    ...
    def rgbToGrayEinsum(self, image):
        return numpy.einsum('...j,j', image[...,:3], [0.299, 0.587, 0.114])

    #This method follows the convention for when you want a python class file to be private
    def _makeBoundingBox(self, startPair, endPair):
        """
    ...
    def cropImage(self, image, startXY, endXY):
        """
        image  : 3D array (/matrix) which is the image
        startXY: user input y-x pair
        endxY  : user input y-x pair
        """
        minX, minY, maxX, maxY = self._makeBoundingBox(startXY, endXY)
